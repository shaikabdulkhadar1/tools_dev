...
const UuidGenerator = () => {
  ...
  const [selectedUuids, setSelectedUuids] = useState<Set<number>>(new Set());
  ...
  const generateTimestampUuid = (): string => {
    ...
    return `${timestamp.substr(0, 8)}-${timestamp.substr(
      8,
      4
    )}-1${random.substr(0, 3)}-${random.substr(3, 4)}-${random.substr(
      7
    )}${Math.random().toString(16).substr(2, 8)}`.substr(0, 36);
  };
  ...
  const generateNameBasedUuid = (hashType: string): string => {
    ...
    const hash = Array.from(name + namespace)
      .reduce((hash, char) => {
    ...
    return `${hash.substr(0, 8)}-${hash.substr(
      8,
      4
    )}-${versionBit}${hash.substr(13, 3)}-${hash.substr(16, 4)}-${hash.substr(
      20,
      12
    )}`.substr(0, 36);
  };
  ...
  const generateUuids = () => {
    ...
    toast({
      title: "Success",
      description: `Generated ${count} UUID${
        count > 1 ? "s" : ""
      } (v${version})`,
    });
  };
  ...
  const toggleUuidSelection = (index: number) => {
  ...
  const deleteSelectedUuids = () => {
    setUuids((prev) => prev.filter((_, index) => !selectedUuids.has(index)));
    setSelectedUuids(new Set());
    toast({
      title: "Deleted",
      description: `Deleted ${selectedUuids.size} UUID${
          ...
          <Card className="p-6 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 hover:shadow-lg transition-all duration-300">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {uuids.map((uuid, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg group hover:shadow-md transition-all duration-200"
                >
                  <div className="flex items-center space-x-3 flex-1">
                    <input
                      type="checkbox"
                      checked={selectedUuids.has(index)}
                      onChange={() => toggleUuidSelection(index)}
